...
public class TransformHandler extends BaseHandler {
    ...
    private void setContentType(Exchange exchange) {
        ...
        if (currentType != null) {
            exchange.getMessage().getContext().setProperty(Exchange.CONTENT_TYPE, currentType).addLabels(BehaviorLabel.TRANSIENT.label());
        } else {
            // make sure no property is used for current scope
            Property p = exchange.getMessage().getContext().getProperty(Exchange.CONTENT_TYPE);
            if (p != null) {
