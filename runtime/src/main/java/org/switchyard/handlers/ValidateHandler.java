...
public class ValidateHandler extends BaseHandler {
    ...
    private Validator<?> get(Exchange exchange) {
        Property contentType = exchange.getMessage().getContext().getProperty(Exchange.CONTENT_TYPE);
        Property validatedType = exchange.getMessage().getContext().getProperty(KEY_VALIDATED_TYPE);
    ...
    private ValidationResult applyValidator(Exchange exchange, Validator validator) {
       ...
       exchange.getMessage().getContext().setProperty(KEY_VALIDATED_TYPE, validator.getType())
