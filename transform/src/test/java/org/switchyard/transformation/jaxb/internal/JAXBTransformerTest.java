...
package org.switchyard.transformation.jaxb.internal;
...
import org.switchyard.transformation.AbstractTransformerTestCase;
import org.switchyard.transformation.config.model.JAXBTransformModel;
import org.switchyard.transformation.jaxb.internal.JAXBMarshalTransformer;
import org.switchyard.transformation.jaxb.internal.JAXBTransformerFactory;
import org.switchyard.transformation.jaxb.internal.JAXBUnmarshalTransformer;
...
public class JAXBTransformerTest extends AbstractTransformerTestCase {
    ...
    public void test_configRead() throws IOException {
        ...
        Assert.assertEquals("java:org.switchyard.transformation.jaxb.internal.POType", unmarshalingTransformer.getTo().toString());

        Transformer marshalingTransformer = getTransformer("switchyard-config-02.xml");
        Assert.assertEquals("java:org.switchyard.transformation.jaxb.internal.POType", marshalingTransformer.getFrom().toString());
        Assert.assertEquals("A", marshalingTransformer.getTo().toString());
    }
    
    @Test
    public void test_configReadContextPath() throws IOException {
        SwitchYardModel config = new ModelPuller<SwitchYardModel>().pull("org/switchyard/transformation/jaxb/internal/switchyard-config-03.xml", JAXBTransformerTest.class);
        for (TransformModel tm : config.getTransforms().getTransforms()) {
            Assert.assertTrue(tm instanceof JAXBTransformModel);
            Assert.assertEquals("org.switchyard.transformation.jaxb.internal", ((JAXBTransformModel)tm).getContextPath());
        }
    }
    ...
    public void test_createMissingFactoryMethodMessage() throws IOException, SAXException {
        ...
        Assert.assertTrue(message.startsWith("JAXB Type 'org.switchyard.transformation.jaxb.internal.USAddress' does not have a JAXBElement factory method defined in org.switchyard.transformation.jaxb.internal.ObjectFactory.  The supported JAXBElement factory methods are for types:"));
    }
